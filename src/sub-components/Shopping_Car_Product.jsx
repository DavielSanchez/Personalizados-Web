...
function Shopping_Car_Product({ product, userId, onProductRemove }) {
    ...
    const [data, setData] = useState(null) // Cambiar a null
    ...
    const fetchProducto = async () => {
        const urlProduct = `${import.meta.env.VITE_API_LINK}/store/products/id/${product.productId}` // Cambié la URL
        try {
    ...
    useEffect(() => {
        if (data && data.productOffer === true) {
    ...
    // Si no hay datos del producto, mostrar un mensaje
    if (!data) {
        return (
            <tr>
